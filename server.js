...
app.post('/update_config', jsonParser, async (request, response) => {
    ...
    fs.readFile('./clewd/clewd.js', 'utf8', function (err, d) {
        if (err) {
            return console.log(err);
        }
        const newData = d.replace(/\$sessionKey=(.*)?\$/g, `$sessionKey=${sessionKey};$`);
        fs.writeFile('./clewd/clewd.js', `${newData}`, function (err) {
            if (err) {
                return console.log(err);
            }
            console.log('clewd.js文件已更新');
        });
    })
})
